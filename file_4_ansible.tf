...
resource "local_file" "ssh_config" {
    content     = <<EOF
    [web]
    web-1.test ansible_ssh_common_args='-o ProxyCommand="ssh -o StrictHostKeyChecking=accept-new -p 22 -W %h:%p -q ${var.password[0]["linux_user"]}@${local.exip_bastion}"'
    web-2.test ansible_ssh_common_args='-o ProxyCommand="ssh -o StrictHostKeyChecking=accept-new -p 22 -W %h:%p -q ${var.password[0]["linux_user"]}@${local.exip_bastion}"'
    [monitor]
    monitor.test ansible_ssh_common_args='-o ProxyCommand="ssh -o StrictHostKeyChecking=accept-new -p 22 -W %h:%p -q ${var.password[0]["linux_user"]}@${local.exip_bastion}"'
    [grafana]
    grafana.test ansible_ssh_common_args='-o ProxyCommand="ssh -o StrictHostKeyChecking=accept-new -p 22 -W %h:%p -q ${var.password[0]["linux_user"]}@${local.exip_bastion}"'
    [elastic]
    elastic.test ansible_ssh_common_args='-o ProxyCommand="ssh -o StrictHostKeyChecking=accept-new -p 22 -W %h:%p -q ${var.password[0]["linux_user"]}@${local.exip_bastion}"'
    [kibana]
    kibana.test ansible_ssh_common_args='-o ProxyCommand="ssh -o StrictHostKeyChecking=accept-new -p 22 -W %h:%p -q ${var.password[0]["linux_user"]}@${local.exip_bastion}"'
    [bastion]
    ${local.exip_bastion}
    EOF
