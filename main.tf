...
resource "yandex_vpc_subnet" "priv-1-subnet-test" {
  name           = "priv-1-subnet-test"
...
resource "yandex_vpc_subnet" "priv-2-subnet-test" {
  name           = "priv-2-subnet-test"
...
resource "yandex_vpc_subnet" "pub-subnet-test" {
  name           = "pub-subnet-test"
  #zone           = "ru-central1-a"
  network_id     = yandex_vpc_network.network-test.id
  v4_cidr_blocks = ["172.16.254.0/24"]
}
### Создание публичной ПОДСЕТИ в зоне для из файла vars.tf
resource "yandex_vpc_subnet" "bastion-subnet-test" {
  name           = "bastion-subnet-test"
  #zone           = "ru-central1-a"
  network_id     = yandex_vpc_network.network-test.id
...
resource "yandex_compute_instance" "bastion" {
  name = "bastion"                          # Имя ВМ
  hostname = "bastion.test"             # Имя сервера, доступен по DNS в VPC
  ...
  network_interface {
    subnet_id = yandex_vpc_subnet.bastion-subnet-test.id
    nat       = true
...
resource "yandex_compute_instance" "nat" {
  ...
  network_interface {
    subnet_id = yandex_vpc_subnet.pub-subnet-test.id
    nat       = true
  }
  metadata = {
    user-data = var.user_data
...
resource "yandex_compute_instance" "web-1" {
  ...
  resources {
   ...
   network_interface {
    subnet_id = yandex_vpc_subnet.priv-1-subnet-test.id
    nat       = false
    security_group_ids = [
...
resource "yandex_compute_instance" "web-2" {
  ...
  network_interface {
    subnet_id = yandex_vpc_subnet.priv-2-subnet-test.id
    nat       = false
    security_group_ids = [
      yandex_vpc_security_group.all_ingress_ssh.id,
      yandex_vpc_security_group.egress_nat.id,
